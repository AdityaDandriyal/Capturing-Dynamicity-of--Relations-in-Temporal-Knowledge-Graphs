...
for k in range(args.freq,30000,args.freq):
	...
	for line in model_output_head:
	    ...
	    for ele in line.split():
	        ...
	        temp_out.append(tup)
	        count = count+1
	    model_out_head.append(temp_out)   #creating a list of list for each line [[line1],[line2]]
	...
	for row in model_out_head:
	    row.sort(key=lambda x:x[0])    #sorting tuples in each line list by its first element
	...
	hit_at_20_tail = tail_array[np.where(tail_array < 20)]
	hit_at_20_head = head_array[np.where(head_array < 20)]

	print('Epoch {} : {}_tail HIT@10 {}\t {}_head HIT@10 {}'.format(k, args.eval_mode, len(hit_at_10_tail)/float(len(tail_array))*100, args.eval_mode, len(hit_at_10_head)/float(len(head_array))*100))

	print('Epoch {} : {}_tail HIT@20 {}\t {}_head HIT@20 {}'.format(k, args.eval_mode, len(hit_at_20_tail)/float(len(tail_array))*100, args.eval_mode, len(hit_at_20_head)/float(len(head_array))*100))
